  ...
  tmpf<-function(dataset,arg1,cname){
   ...
   tmp<-subset(tmp,tmp$Search.Volume >1000 | tmp$Search.Volume >= 500)
    ...
    for(t in t2){
      ...
      if(track >= 50) { #country.searchvol.positive.keyword
       ggsave(filename = paste0("figs/search.vol/country/",cname,".",round(track),".","positive",".",t,".png"),timeseries_calc(nats,t,cname))}
        else if(track<= -50) { 
          ggsave(filename = paste0("figs/search.vol/country/",cname,".",round(track),".","negative",".",t,".png"),timeseries_calc(nats,t,cname))}
...
timeseries_calc<-function(doc,arg2,cname,pm=5){
  ...
  ps<-plot_timeseries(forec.20,format.date = T)+
    ...
    labs(x="TimeSeries")+
    ggtitle(bquote(atop(.(paste0("Keyword :- ",arg2)), atop(.(paste0("Region:-",cname)), ""))))+
    theme(axis.text.x=element_text(angle=90, hjust=1))
